import CodexJson from "../schema/codex.json";
import SizesJson from "../schema/sizes.json";
import CounterJson from "../schema/counter.json";
import DefaultSizesJson from "../schema/default_sizes.json";
...
async function get_json(uri) {
  ...
  if (!response.ok)
    throw new Error(`GET ${uri}: ${response.status ?? "failed"}`);
 ...
 * @returns {object}
 */
async function transform_patterns_promise(default_sizes_promise) {
  const default_sizes = await default_sizes_promise;
  return transform_pattern(default_sizes);
}
 ...
 * @returns {object}
 */
async function transform_patterns(default_sizes) {
...
export class CesrTables {
  ...
  static async load() {
    ...
    const default_sizes = transform_patterns(DefaultSizesJson);
    const tables = new CesrTables();
    tables.codex = Object.freeze(await CodexJson);
    tables.sizes = Object.freeze(await SizesJson);
    tables.counter = Object.freeze(await CounterJson);
